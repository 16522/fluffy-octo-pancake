...
class SymmetricPendulumWithPBPrime(Scene):
    def construct(self):
        ...
        # 组装场景元素（添加title到首位确保层级）
        self.add(
            title,  # 先添加标题确保在最上层
            base_line, A, B, B_prime,
            connections, P, P_label,
            A_label, B_label, B_prime_label,
            symmetry_line, a_b_prime_line, 
            perpendicular_symbol, pb_prime_line,
            inequality
        )
